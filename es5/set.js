...
(function (G, F) {
	...
	Set.prototype.elemManip = function (node) {
		...
		var elemTag = elem.splice(0, 1)[0]; // update passed node by removing first tag

		node = node.replace(/<[^>]*>/, "").replace(/<\/[^>]*>$/, ""); // create tag

		var _this$createTag$ = _slicedToArray(this.createTag([1], [elemTag])[0], 1),
			newNode = _this$createTag$[0]; // set properties to newNode
